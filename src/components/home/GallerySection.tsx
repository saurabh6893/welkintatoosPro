...
const GALLERY_IMAGES = [
  { src: "https://images.unsplash.com/photo-1598371839696-5c5bb00bdc28?auto=format&fit=crop&q=80&w=800", title: "Ethereal Lines" },
  { src: "https://images.unsplash.com/photo-1611501275019-9b5cda994e8d?auto=format&fit=crop&q=80&w=800", title: "Metric Flow" },
  { src: "https://images.unsplash.com/photo-1562962230-16bc46364924?auto=format&fit=crop&q=80&w=800", title: "Noir Portrait" },
  { src: "https://images.unsplash.com/photo-1590246296840-e6f72c427673?auto=format&fit=crop&q=80&w=800", title: "Abstract Form" },
  { src: "https://images.unsplash.com/photo-1565415733475-6804561726a9?auto=format&fit=crop&q=80&w=800", title: "Nature's Echo" },
];
...
export default function GallerySection() {
           ...
           itemsRef.current.forEach((item, index) => {
               if(item) {
                   ...
                   tl.fromTo(item, 
                       ...
                       { filter: "grayscale(0%)", scale: 1.05, ease: "none", duration: 0.5 }
                   )
                   .to(item, 
                       { filter: "grayscale(100%)", scale: 1, ease: "none", duration: 1 }
                   );
               }
           });
        }
  ...
  return (
    <section id="gallery" ref={triggerRef} className="relative h-screen overflow-hidden bg-black text-white">
       {/* Section Header */}
        <div className="absolute top-10 left-10 z-10 mix-blend-difference">
            <h2 className="text-sm uppercase tracking-widest font-bold">Selected Works</h2>
        </div>
      ...
      >
        <div className="w-[50vw] flex flex-col justify-center">
             <h3 className="text-6xl md:text-8xl font-bold font-sans leading-none tracking-tight">
                 Ink as <br /> <span className="text-white/50 italic font-serif">Narrative.</span>
             </h3>
             <p className="mt-8 max-w-md text-white/60 font-light">
                 Each piece is a collaboration, a unique story translated into permanent art.
             </p>
        </div>
        ...
        {GALLERY_IMAGES.map((img, index) => (
          ...
          >
            ...
            <div className="absolute bottom-6 left-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-y-4 group-hover:translate-y-0 mobile-active-label">
                <span className="text-xs uppercase tracking-widest font-bold">{img.title}</span>
            </div>
          </div>
        ))}
